...
uint8_t Telemetry::configUpdate(const bool save)
{
    ...
    // calculate the maximum bandwith of current setting. guarantee the serial port could handle the data
...
void Telemetry::setMaxSpeed(float max_speed)
{
    Header header;
    header.data.LEN = sizeof(OSDK_Uart_Header_t) + 
        sizeof(float) + OSDK_MISC_SIZE;
    header.data.SESSION = 0;
    header.data.ACK = 0;
    header.data.SEQ = vehicle->hal->serial_getSeq();
    header.append_crc();

    vehicle->hal->serialSend(header.data,
        OSDK_INIT_SET, OSDK_SET_MAX_SPEED_ID,
        &max_speed, sizeof(float));
}
...
void Telemetry::SerialHandle(void)
{
    while(true)
    {
        ...
        //process timestamp  
        this->timestamp = *((OSDK_Push_Data_Timestamp_t*)pData);
        ...
        if(flag->motor)
        {
            ...
            float left_knee_angle = (motors.left_knee.pos + (motors.left_knee.rev * 2*3.14)) / 2 ;
            left_leg_length = cosf(left_knee_angle)*0.28;

            float right_knee_angle = (motors.right_knee.pos + (motors.right_knee.rev * 2*3.14)) / 2 ;
            right_leg_length = cosf(right_knee_angle)*0.28;
